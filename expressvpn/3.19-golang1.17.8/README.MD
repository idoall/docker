...
cd expressvpn/3.19-golang1.17.8

# hack hack hack

# build
docker build -t idoall/expressvpn3.19-golang1.17.8 .
```
...
`docker pull idoall/expressvpn3.19-golang1.17.8`
...
## Start the container
    ...
    docker run \
      ...
      --tty=true \
      --name=expressvpn \
      idoall/expressvpn3.19-golang1.17.8 \
      /bin/bash
...
In this case all traffic is routed via the vpn container. To reach the other containers locally the port forwarding must be done in the vpn container (the network mode service does not allow a port configuration)
  ...
  expressvpn:
    ...
    image: idoall/expressvpn3.19-golang1.17.8
    environment:
